...
func (r *Reconciler) FinalizeKind(ctx context.Context, original *v1alpha1.KnativeEventing) pkgreconciler.Event {
	...
	manifest, err := common.InstalledManifest(ctx, original, common.Parent(r.manifest), r.transform)
	if err != nil {
		logger.Error("Unable to fetch correct manifest, cluster-scoped resources may not be finalized", err)
		return nil
	}
...
func (r *Reconciler) ReconcileKind(ctx context.Context, ke *v1alpha1.KnativeEventing) pkgreconciler.Event {
	...
	stages := common.ReconcileStages{
		common.Parent(r.manifest),
		r.transform,
	...
	if _, err := common.TargetManifest(ctx, ke, stages...); err != nil {
		return err
	}
...
func (r *Reconciler) transform(ctx context.Context, manifest *mf.Manifest, obj v1alpha1.KComponent) error {
	logger := logging.FromContext(ctx)
	instance := obj.(*v1alpha1.KnativeEventing)
